...
global initial_flag
persistent fhd f_bias %#ok<USENS>
% benchmark_func.m is the main function for 25 test functions, all minimization problems
...
if initial_flag==0
    ...
    elseif func_num==54, fhd=str2func('shifted_ackley_func'); %[-32,32]
...
if initial_flag==0
    ...
    elseif D==50,load elliptic_M_D50,
        ...
        A=MyNormrnd(0,1,D,D);[M,~]=cGram_Schmidt(A);
    end
    initial_flag=1;
    global xSol
xSol=o;
...
f=f.*(1+0.4.*abs(MyNormrnd(0,1,ps,1)));
...
[ps,~]=size(x);
fit=hybrid_rot_func1(x).*(1+0.2.*abs(MyNormrnd(0,1,ps,1)));
%----------------------------------------------------------------
...
xSol=o(1,1:D);
    ...
    lamda=repmat(lamda,1,D);
    c=[2 3 2 3 2 3 20 30 200 300]; %#ok<NASGU>
...
fit=hybrid_composition_func(x,fun_num,func,o,sigma,lamda,bias,M);
%----------------------------------
%   25.	Rotated Hybrid Composition Function without Bounds	
...
function fit=shifted_ackley_func(x)
global initial_flag
persistent o M
...
[ps,~]=size(x);
f=sum(x.^2,2).*(1+0.1.*MyNormrnd(0,1,ps,1));
%--------------------------------
...
M=P*D*Q;
%--------------------------------
function [x] = MyNormrnd(mu,sigma,m,n)
